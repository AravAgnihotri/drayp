...
const METRIC_CARDS = [
  ...
  { label: "Best Category",   value: "Tops", delta: null,   positive: null,  sub: "Highest avg score"   },
        ...
        {METRIC_CARDS.map((m) => (
          <div key={m.label} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-2 flex items-end gap-2">
              ...
              {m.delta && (
                <span className={cn("mb-1 text-xs font-semibold", m.positive ? "text-[#0F6E56]" : "text-rose-600")}>
                  {m.delta}
                </span>
              )}
            </div>
            ...
            {FIT_HISTORY.map((entry, i) => (
              <li key={i} className="relative flex items-start gap-4">
                <div className={cn(
                  ...
                  entry.returned ? "border-gray-300" : "border-[#1D9E75]"
                )}>
                  <div className={cn(
                ...
                <div className="flex-1 pt-px">
                  <div className="flex items-center justify-between gap-2">
                    <p className={cn("text-sm font-medium", entry.returned ? "text-gray-400" : "text-gray-800")}>
                      {entry.item}
                    </p>
                    {entry.returned ? (
